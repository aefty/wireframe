...
module.exports = function(workFlow, procsObj) {
    ...
    var _merge = function (results,callback){
        
        _assembleTask(workFlow[meth][url].merg, function(err, task) {
            ...
            var d  =  function (next){ next(null,results);};
            taskArr.unshift(d);
        ...
        if (!(req.method in workFlow) && !('*' in workFlow)) {
            ...
            _urlMatch(req.url, workFlow[meth], function(match) {
                if (match === null) {
                    ...
                    async.parallel(branch, function (err,results){
                        
                        return merg(results,callback)
