 ...
 module.exports = function(struc, procs) {
    ...
    var waterfall = function(callback) {
       ...
       var stack = [];
       for (var key in Structure[request.method][url_match].sync) {
          var module = Structure[request.method][url_match].sync[key]
             .split(".");
    ...
    var parallel = function(callback) {
       ...
       for (var key in Structure[request.method][url_match].async) {
          var module = Structure[request.method][url_match].async[key]
             .split(".");
    ...
    this.run = function(req, callback) {
       ...
       if (request.method in Structure || request.method.toLowerCase() in Structure) {
          for (var url in Structure[request.method]) {
             if (urlglob(url, request.url)) {
                url_match = url;
                var branch = {};
                if (typeof Structure[request.method][url_match].sync != "undefined")
                   branch.sync = waterfall;
                if (typeof Structure[request.method][url_match].async != "undefined")
                   branch.async = prallel;
